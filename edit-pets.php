<?php

require "config.php";

use App\Pets;

$pet_id = $_GET['id'];

$pet = Pets::getById($pet_id);
?>
<!DOCTYPE html>
<html>

<head>
	<meta charset="utf-8">
	<meta name="viewport" content="width=device-width, initial-scale=1">
	<title>Edit Pets</title>
</head>

<body>
	<h1>Edit Pet</h1>

	<form action="save-changes.php" method="POST">
		<input type="hidden" name="id" value="<?php echo $pet->getId(); ?>">
		<div>
			<label>Pet Name</label>
			<input type="text" name="pet_name" placeholder="Pet Name" value="<?php echo $pet->getPetName(); ?>" />
		</div>
		<div>
			<label>Pet Gender</label>
			<input type="text" name="pet_gender" placeholder="Pet Gender" value="<?php echo $pet->getPetGender(); ?>" />
		</div>
		<div>
			<label>Pet Birthdate</label>
			<input type="text" name="pet_birthdate" placeholder="Pet Birthdate" value="<?php echo $pet->getPetBirthdate(); ?>" />
		</div>
		<div>
			<label>Pet owner</label>
			<input type="text" name="pet_owner" placeholder="Pet owner" value="<?php echo $pet->getPetOwner(); ?>" />
		</div>
		<div>
			<label>Email Address</label>
			<input type="email" name="email" placeholder="email@address.com" value="<?php echo $pet->getEmail(); ?>" />
		</div>
		<div>
			<label>Address</label>
			<input type="text" name="address" placeholder=" Address " value="<?php echo $pet->getAddress(); ?>" />
		</div>
		<div>
			<label>Contact Number</label>
			<input type="text" name="contact_number" placeholder="Contact Number" value="<?php echo $pet->getContactNumber(); ?>" />
		</div>
		<div>
			<button>
				Save
			</button>
			<a href="index.php">Cancel</a>
		</div>
	</form>
</body>

</html>